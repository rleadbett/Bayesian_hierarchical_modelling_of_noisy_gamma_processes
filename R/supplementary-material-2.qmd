---
title: "Fitting noisy gamma processes using RStan"
subtitle: "Leadbetter, R.L., Gonzalez Caceres, G., and Phatak, A."
author: 
...
htmltools::a(
    href = "https://www.maintenance.org.au/dashboard.action",
    htmltools::img(src = knitr::image_uri("..//figures//ctmtds-logo-small.png")))
if (interactive()) {
...
```{r echo=FALSE}
#| label: plot-underlying-paths
#| echo: false
# Plotting function 2
# NB Added argument for data frame
...
This document contains the supplementary material for Section 5 (Modelling unit-to-unit variability) of the manuscript Leadbetter, R.L, Gonzalez Caceres, G. J. and Phatak, A. (2024) *Fitting noisy gamma processes using RStan*.
...
pdf(
  file.path("..", "figures", "RP_w_noise.pdf"),
  height = 6,
  width = 8
)
p_noisy_crackgrowth
...
@fig-cp-posterior (a) shows the marginal distributions of the parameters $\mu$, $\nu$, and $\sigma$ and @tbl-marginal-posteriors-cp shows the sampling chain diagnostics from the sampling. The complete pooling model has done a good job of recovering the true value of $\sigma$ ($0.025$). There also does not appear to be any degenerate behaviour in the posteriors of the parameters [@fig-cp-posterior (b)].
...
tbl_draws_cp %>%
  kbl(
    booktabs = T,
    format = "latex",
    caption = "Output from fitting a model with complete pooling to the noisy data of Fig.~\\ref{fig:RP_w_noise}. We assume that the data from all units is a manifestation of a single underlying gamma process, and hence the mean and coefficient of variation of the process do not vary from unit-to-unit.",
    escape = FALSE,
    label = "cp"
  ) %>%
...
Once again, we sample $6000$ draws from the posterior, using six chains, each $2000$ draws long with a burn-in of $1000$. Now, some divergent transitions crop up with the extra hierarchical layer in the model. However, there are very few divergencies, and there doesn't appear to be any structure in their locations (see @fig-pairs-pp-mu.) From @tbl-post-pp-mu and @fig-marginal-pp-mu, we can see that this model is able to recover the true value of the standard deviation of the measurement error to the same extent as the complete pooling model in the previous section.
...
tbl_draws_pp_mu %>%
  kbl(
    booktabs = T,
    format = "latex",
    caption = "Partial output from fitting a BHM to the noisy data of Fig.~\\ref{fig:RP_w_noise} where mean degradation $\\mu_j$ varies between units. Only statistics for Units~1--4 are shown.",
    escape = FALSE,
    label = "pp_mu"
  ) %>%
  kable_styling(latex_options = "striped") %>%
...
pdf(
  file.path("..", "figures", "plot-pp-mu-marg-post-1.pdf"),
  height = 6,
  width = 6
)
plotMarginals(stan_fit_pp_mu)
...
The varying $\mu$ model also recovers the true degradation path of each unit (@fig-pp-mu-posterior-paths).
...
pdf(
  file.path("..", "figures", "plot-pp-mu-filtered.pdf"),
  height = 6,
  width = 8
)
plotReclaimedPaths(
...
tbl_draws_pp_nu %>%
  kbl(
    booktabs = T,
    format = "latex",
    caption = "Partial output from fitting a BHM to the noisy data of Fig.~\\ref{fig:RP_w_noise} where the coefficient of variation $\\nu_j$ varies between units. Only statistics for Units~1--4 are shown.",
    escape = FALSE,
    label = "pp_nu"
  ) %>%
  kable_styling(latex_options = "striped") %>%
...
# Partial pooling on both $\mu$ and $\nu$ {#sec-pp-both}

The final model includes partial pooling of both $\mu$ and $\nu$. It uses the same hyperpriors as we use in the two models above in  @sec-pp-mu and @sec-pp-nu,
...
The model with varying $\mu$ and $\nu$ suffers from the same sampling issues as the varying $\nu$ model but much worse. However, the model still recovers the true parameter value of $\sigma$, and it appears to fit the same unit-specific values of $\mu$ as the varying $\mu$ model in @sec-pp-mu.
...
tbl_draws_pp_both %>%
  kbl(
    booktabs = T,
    format = "latex",
    caption = "Partial output from fitting a BHM to the noisy data of Fig.~\\ref{fig:RP_w_noise} where both the coefficient of variation $\\nu_j$ and the mean wear rate $\\mu_j$ varies between units. Only statistics for Units~1--4 are shown.",
    escape = FALSE,
    label = "pp_both"
  ) %>%
  kable_styling(latex_options = "striped") %>%
...
We now repeatedly apply the function to calculate $elppd$ for each unit in the dataset to get the $\hbox{elppd}_{LOUO-CV}$ for each model. The results are displayed in @tbl-elppd-louo.
...
We now recursively fit each model to calculate $\hbox{elppd}_\hbox{\tiny{SA-CV}}$ and display the results in @tbl-elppd-sa.
...
data.frame(
  ...
  kbl(
    booktabs = T,
    format = "latex",
    caption = "Leave-one-out cross-validation statistics for the models fitted in Section~\\ref{subsec:fitunitsmean}.",
    escape = FALSE,
    label = "elppd_loo"
  ) %>%
  kable_styling(latex_options = "striped") %>%
